  ...
  try {
    ...
    if (trades.length === 0) {
      return NextResponse.json({ 
        success: false, 
        error: 'Không có giao dịch nào để phân tích tuần.' 
      });
    }
    ...
    const prompt = `
Bạn là một YUP Trade cao cấp. Hãy nhận xét hiệu suất giao dịch tuần vừa qua của trader dựa trên danh sách các lệnh dưới đây:

Danh sách lệnh giao dịch gần đây:
${JSON.stringify(tradesContext, null, 2)}

Hãy viết một báo cáo nhận xét tuần bằng tiếng Việt định dạng JSON. Báo cáo cần bao quát xu hướng tâm lý, tính kỷ luật, điểm mạnh/yếu nổi bật, bài học cốt lõi, và đưa ra kế hoạch hành động cụ thể cho tuần tới.

Yêu cầu đầu ra JSON chính xác với cấu trúc:
{
  "summary": "Nhận xét tổng quan về hiệu suất giao dịch tuần này (khoảng 3-4 câu).",
  "discipline_score": 7.5, // Điểm số kỷ luật tuần này từ 1.0 đến 10.0 (số thực)
  "strengths": ["Mảng 3-4 điểm mạnh/quyết định đúng đắn nổi bật tuần qua."],
  "weaknesses": ["Mảng 3-4 lỗi sai/yếu tố tâm lý cần khắc phục tuần qua."],
  "key_lessons": ["Mảng 3-4 bài học đắt giá rút ra được từ kết quả lệnh."],
  "action_plan": ["Mảng 3-4 hành động cụ thể trader phải tuân thủ trong tuần tiếp theo để cải thiện."]
}
...
`;
    ...
    for (const model of models) {
      try {
        console.log(`Weekly review: attempting with model: ${model}`);
