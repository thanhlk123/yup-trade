...
import { Target, TrendingDown, Filter, X, Flame, Lightbulb, Zap, Search, BrainCircuit, Swords, AlertOctagon } from 'lucide-react';
...
export function MartingaleDetail({ behavior, onFilterTrades, onClose, trades, t }) {
    ...
    <div className="mt-6 rounded-2xl border theme-border bg-white dark:bg-slate-900 shadow-xl dark:shadow-2xl relative overflow-hidden animate-slide-up">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 text-xs font-semibold">
              ...
              <span className="px-2.5 py-1 rounded-md bg-rose-50 text-rose-600 dark:bg-rose-500/10 dark:text-rose-300">
                Mức độ nghiêm trọng <span className="font-black text-rose-700 dark:text-rose-400">{severity || '9.5'} / 10</span>
              </span>
            </div>
          </div>
      ...
      <div className="p-6 space-y-6 relative z-10 bg-slate-50/50 dark:bg-slate-900/50">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {/* Left Box: Stats */}
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 border theme-border shadow-sm flex flex-col justify-between items-center text-center relative overflow-hidden">
            {/* Warning background glow */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-32 h-32 bg-rose-500/10 blur-3xl rounded-full pointer-events-none"></div>
            ...
            <div className="relative w-40 h-40 flex items-center justify-center mb-4">
               ...
               <svg className="absolute inset-0 w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                 ...
                 {/* Foreground Circle */}
                 <circle cx="50" cy="50" r="45" fill="none" stroke="currentColor" strokeWidth="6" strokeDasharray={`${violationPct * 2.83} 283`} className="text-rose-600 transition-all duration-1000 ease-out" strokeLinecap="round" />
               </svg>
               <div className="relative z-10 flex flex-col items-center">
                 <span className="text-4xl font-black text-slate-900 dark:text-white">{violationCount}</span>
                 <span className="text-[11px] font-bold text-slate-500 dark:text-slate-400">lệnh bị ảnh hưởng</span>
            ...
            <div>
              <div className="flex items-center gap-2.5 mb-3">
                 ...
                 <h5 className="text-sm font-bold text-slate-800 dark:text-slate-200">
                   Con đường nhanh nhất để cháy tài khoản.
                 </h5>
              </div>
              <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
                <strong className="text-slate-800 dark:text-slate-200">{violationCount} lệnh ({violationPct}%)</strong> có dấu hiệu gấp thếp (Martingale). Sau khi cắn Stop Loss, bạn đã ngay lập tức nhồi rủi ro/khối lượng lớn hơn vào cùng một mã giao dịch với ảo tưởng "gỡ lại vốn".
              </p>
            </div>
                    ...
                    return (
                      <div key={m} className="flex flex-col items-center justify-end h-full flex-1 group">
                        <div className="w-full flex justify-center items-end h-full pb-2">
                           <div 
                             className={`w-full max-w-[24px] rounded-t-sm transition-all duration-300 ${isHigh || val === maxTrend ? 'bg-rose-600' : 'bg-rose-300/60 dark:bg-rose-500/40 hover:bg-rose-400'}`}
                             style={{ height: `${hPct}%` }}
                           />
                        </div>
        ...
        <div className="bg-slate-100/50 dark:bg-slate-800/50 rounded-2xl p-6 border theme-border">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
             <div className="flex items-start gap-4">
                ...
                <div>
                  <h6 className="font-bold text-slate-800 dark:text-slate-200 mb-1.5 text-sm">Ảo tưởng gỡ gạc</h6>
                  <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">Não bộ không chịu chấp nhận lệnh thua, khao khát đòi lại tiền ngay lập tức dẫn đến việc nhồi lệnh gấp bội, biến rủi ro thành trò chơi đỏ đen.</p>
                </div>
             </div>
             <div className="flex items-start gap-4">
                <div className="p-3 rounded-full bg-rose-100 dark:bg-rose-500/20 text-rose-600 dark:text-rose-400 shrink-0">
                  <Flame className="w-6 h-6" />
                </div>
                <div>
                  <h6 className="font-bold text-slate-800 dark:text-slate-200 mb-1.5 text-sm">Khuếch đại rủi ro kép</h6>
                  <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">Nếu tiếp tục sai, khoản lỗ không chỉ cộng dồn mà còn phình to theo cấp số nhân (Loss Amplified). Đây là nguyên nhân số 1 gây "cháy tài khoản".</p>
                </div>
             </div>
             <div className="flex items-start gap-4">
                <div className="p-3 rounded-full bg-violet-100 dark:bg-violet-500/20 text-violet-600 dark:text-violet-400 shrink-0">
                ...
                <div>
                  <h6 className="font-bold text-slate-800 dark:text-slate-200 mb-1.5 text-sm">Phá vỡ kỷ luật gốc</h6>
                  <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">Khi đã gấp thếp, bạn thường quăng mọi Setup / Confluence qua cửa sổ. Điểm vào lệnh chỉ mang tính cảm tính bừa bãi.</p>
                </div>
             </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 border theme-border shadow-sm">
             <div className="flex items-center gap-2 mb-5 text-slate-500 dark:text-slate-400 font-black text-xs uppercase tracking-widest">
                <Search className="w-4 h-4" /> BẰNG CHỨNG
             </div>
             <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="bg-slate-50 dark:bg-slate-900/50 rounded-xl p-4 border border-slate-100 dark:border-slate-700/50 space-y-1">
                       {evObserved[0].split('\n').map((line, idx) => {
                         ...
                         const isRedAlert = line.includes('RỦI RO KÉP') || line.includes('thất bại');
                         return (
                           <div key={idx} className={`text-sm leading-relaxed ${isBullet ? `ml-2 mt-2 ${isRedAlert ? 'text-rose-600 dark:text-rose-400' : 'text-slate-600 dark:text-slate-400'} flex items-start gap-2` : 'font-semibold text-slate-800 dark:text-slate-200 mb-3'}`}>
                              {isBullet && <span className={`${isRedAlert ? 'text-rose-500' : 'text-blue-500'} font-black mt-0.5`}>•</span>}
                ...
                {evContext.length > 0 && (
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2 text-sm font-bold text-slate-800 dark:text-slate-200">
                         <span className="w-2 h-2 rounded-full bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.8)]"></span> Bối cảnh tâm lý & rủi ro (Context)
                      </div>
          ...
          <div className="bg-rose-50/50 dark:bg-rose-900/20 rounded-2xl p-6 border border-rose-100 dark:border-rose-500/20 shadow-sm flex flex-col justify-between">
             ...
             <div className="flex gap-3 text-rose-900 dark:text-rose-100 flex-1 items-center">
                ...
                <p className="text-sm font-medium leading-relaxed italic pr-4">
                  Một lệnh thua không phải là tận thế, nhưng một chuỗi gấp thếp thất bại chắc chắn là dấu chấm hết cho tài khoản của bạn. Đừng bao giờ lấy rủi ro lớn hơn để trả đũa thị trường.
                </p>
             </div>
        ...
        <div className="bg-slate-100 dark:bg-slate-800/60 rounded-xl p-4 border theme-border flex items-center gap-3">
           ...
           <div className="text-xs text-slate-600 dark:text-slate-300 font-medium flex items-center flex-wrap gap-x-2 gap-y-1">
             <span>Nghỉ giao dịch ít nhất 1 buổi nếu thua 2 lệnh liên tiếp</span>
             <span className="text-slate-400">•</span>
             <span>Ghi quy tắc Max Risk Per Trade ra giấy ghi chú dán trên màn hình</span>
           </div>
        </div>
