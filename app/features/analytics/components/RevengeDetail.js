...
import { Target, TrendingDown, Filter, X, Flame, Lightbulb, Zap, Search, BrainCircuit, Swords } from 'lucide-react';
...
export function RevengeDetail({ behavior, onFilterTrades, onClose, trades, t }) {
      ...
      <div className="p-6 space-y-6 relative z-10 bg-slate-50/50 dark:bg-slate-900/50">
            ...
            <div>
              <div className="flex items-center gap-2.5 mb-3">
                 ...
                 <h5 className="text-sm font-bold text-slate-800 dark:text-slate-200">
                   Bạn đang để cảm xúc điều khiển cuộc chơi.
                 </h5>
              </div>
              <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
                <strong className="text-slate-800 dark:text-slate-200">{violationCount} lệnh ({violationPct}%)</strong> có dấu hiệu trả thù thị trường. Khi cố gắng "đòi lại" những gì vừa mất bằng cách vào lệnh liên tục và tăng khối lượng, bạn đang đánh bạc thay vì giao dịch.
              </p>
            </div>
        ...
        <div className="bg-slate-100/50 dark:bg-slate-800/50 rounded-2xl p-6 border theme-border">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
             <div className="flex items-start gap-4">
                ...
                <div>
                  <h6 className="font-bold text-slate-800 dark:text-slate-200 mb-1.5 text-sm">Vòng xoáy tử thần</h6>
                  <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">Thua lỗ &rarr; Cảm xúc bùng phát &rarr; Tăng rủi ro để gỡ &rarr; Thua lớn hơn. Đây là con đường nhanh nhất dẫn đến cháy tài khoản.</p>
                </div>
             </div>
             <div className="flex items-start gap-4">
                <div className="p-3 rounded-full bg-violet-100 dark:bg-violet-500/20 text-violet-600 dark:text-violet-400 shrink-0">
                ...
                <div>
                  <h6 className="font-bold text-slate-800 dark:text-slate-200 mb-1.5 text-sm">Mất kiểm soát hoàn toàn</h6>
                  <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">Quyết định của bạn không còn dựa trên xác suất hay setup, mà hoàn toàn do adrenaline và sự hối tiếc chi phối.</p>
                </div>
             </div>
             <div className="flex items-start gap-4">
                <div className="p-3 rounded-full bg-violet-100 dark:bg-violet-500/20 text-violet-600 dark:text-violet-400 shrink-0">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">Giao dịch trả thù thường kéo theo chuỗi vi phạm khác: thả trôi Stop Loss, nhồi lệnh vô tội vạ, giao dịch quá mức.</p>
                </div>
             </div>
          </div>
        </div>
                  ...
                  <div>
                    ...
                    <div className="bg-slate-50 dark:bg-slate-900/50 rounded-xl p-4 border border-slate-100 dark:border-slate-700/50 space-y-1">
                       {evObserved[0].split('\n').map((line, idx) => {
                         ...
                         return (
                           <div key={idx} className={`text-sm leading-relaxed ${isBullet ? 'ml-2 mt-2 text-slate-600 dark:text-slate-400 flex items-start gap-2' : 'font-semibold text-slate-800 dark:text-slate-200 mb-3'}`}>
          ...
          <div className="bg-rose-50/50 dark:bg-rose-900/20 rounded-2xl p-6 border border-rose-100 dark:border-rose-500/20 shadow-sm flex flex-col justify-between">
             ...
             <div className="flex gap-3 text-rose-900 dark:text-rose-100 flex-1 items-center">
                ...
                <p className="text-sm font-medium leading-relaxed italic pr-4">
                  Thị trường không nợ bạn bất cứ điều gì. Cố gắng "đòi lại" tiền ngay lập tức sau lệnh thua chỉ làm mọi thứ tệ hơn. Hãy đứng dậy, uống một ly nước, và chỉ quay lại khi não bộ đã "nguội" hoàn toàn.
                </p>
             </div>
        ...
        <div className="bg-slate-100 dark:bg-slate-800/60 rounded-xl p-4 border theme-border flex items-center gap-3">
           ...
           <div className="text-xs text-slate-600 dark:text-slate-300 font-medium flex items-center flex-wrap gap-x-2 gap-y-1">
             <span>Áp dụng quy tắc nghỉ bắt buộc 30 phút sau lệnh thua</span>
             <span className="text-slate-400">•</span>
             <span>Cài đặt cứng Daily Loss Limit trên phần mềm</span>
             <span className="text-slate-400">•</span>
             <span>Viết ra giấy lý do vào lệnh trước khi click chuột</span>
           </div>
        </div>
