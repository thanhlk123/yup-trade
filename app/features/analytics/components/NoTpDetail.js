...
import { Target, TrendingDown, Filter, X, Frown, Lightbulb, Zap, Search, MessageSquare, ListTree, AlertCircle } from 'lucide-react';
...
export function NoTpDetail({ behavior, onFilterTrades, onClose, trades, t }) {
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <h4 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight mb-3">
              Không có kế hoạch chốt lời
            </h4>
            <div className="flex items-center gap-2 text-xs font-semibold">
              <span className="px-2.5 py-1 rounded-md bg-indigo-50 text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-300 uppercase">
                CẤP {category || 'Execution'}
              </span>
              <span className="text-slate-300 dark:text-slate-600">•</span>
              <span className="px-2.5 py-1 rounded-md bg-rose-50 text-rose-600 dark:bg-rose-500/10 dark:text-rose-300">
                Mức độ nghiêm trọng <span className="font-black">{severity || '5.5'} / 10</span>
              </span>
            </div>
          </div>
      ...
      <div className="p-6 space-y-6 relative z-10 bg-slate-50/50 dark:bg-slate-900/50">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 border theme-border shadow-sm flex flex-col justify-between items-center text-center">
            {/* Circular representation (Simplified via CSS) */}
            <div className="relative w-40 h-40 flex items-center justify-center mb-4">
               ...
               <svg className="absolute inset-0 w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                 ...
                 {/* Foreground Circle - Assuming 30% for visual */}
            ...
            <div>
              <div className="flex items-center gap-2.5 mb-3">
                 ...
                 <h5 className="text-sm font-bold text-slate-800 dark:text-slate-200">
                   Bạn đang giao dịch thiếu kế hoạch chốt lời.
                 </h5>
              </div>
              <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
                <strong className="text-slate-800 dark:text-slate-200">{violationCount} lệnh ({violationPct}%)</strong> không có TP rõ ràng, khiến lợi nhuận của bạn dễ bị cảm xúc chi phối và bỏ lỡ cơ hội tốt.
              </p>
            </div>
            ...
            <div className="mt-6">
               ...
               <div className="flex items-end gap-3 h-24">
                  {trendMonths.length > 0 ? trendMonths.map((m, i) => {
                    ...
                    const isHigh = i === trendMonths.length - 2; // Arbitrary highlight for visual match
                    return (
        ...
        <div className="bg-slate-100/50 dark:bg-slate-800/50 rounded-2xl p-6 border theme-border">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
             <div className="flex items-start gap-4">
                ...
                <div>
                  <h6 className="font-bold text-slate-800 dark:text-slate-200 mb-1.5 text-sm">Dễ bị gồng lệnh</h6>
                  <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">Không có TP &rarr; dễ giữ lệnh quá lâu và trả lại lợi nhuận.</p>
                </div>
             </div>
             <div className="flex items-start gap-4">
                <div className="p-3 rounded-full bg-violet-100 dark:bg-violet-500/20 text-violet-600 dark:text-violet-400 shrink-0">
                ...
                <div>
                  <h6 className="font-bold text-slate-800 dark:text-slate-200 mb-1.5 text-sm">Thoát lệnh cảm tính</h6>
                  <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">Thiếu điểm chốt lời &rarr; dễ thoát sớm hoặc thoát ở vùng không tối ưu.</p>
                </div>
             </div>
             <div className="flex items-start gap-4">
                <div className="p-3 rounded-full bg-violet-100 dark:bg-violet-500/20 text-violet-600 dark:text-violet-400 shrink-0">
                ...
                <div>
                  <h6 className="font-bold text-slate-800 dark:text-slate-200 mb-1.5 text-sm">Hiệu suất bị ảnh hưởng</h6>
                  <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">Thiếu kế hoạch rõ ràng làm giảm khả năng kiểm soát kết quả.</p>
                </div>
             </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {/* Coach Tip */}
          <div className="bg-indigo-50/50 dark:bg-indigo-900/20 rounded-2xl p-6 border border-indigo-100 dark:border-indigo-500/20 shadow-sm flex flex-col justify-between">
             <div className="flex items-center gap-2 mb-4 text-indigo-500 dark:text-indigo-400 font-black text-xs uppercase tracking-widest">
                <Lightbulb className="w-4 h-4" /> COACH TIP
             </div>
             <div className="flex gap-3 text-indigo-900 dark:text-indigo-100 flex-1 items-center">
                <span className="text-4xl text-indigo-300 dark:text-indigo-600 font-serif leading-none mt-[-10px]">"</span>
                <p className="text-sm font-medium leading-relaxed italic pr-4">
                  Luôn xác định vùng TP trước khi vào lệnh.<br/>
                  Kế hoạch không phải để bạn đúng 100% mỗi lệnh, mà để bạn đúng hơn về dài hạn.
                </p>
             </div>
             <div className="text-right text-xs font-bold text-indigo-400 dark:text-indigo-500 mt-4">
               — Coach AI
             </div>
          </div>
        </div>
        ...
        <div className="bg-slate-100 dark:bg-slate-800/60 rounded-xl p-4 border theme-border flex items-center gap-3">
           ...
           <div className="text-xs text-slate-600 dark:text-slate-300 font-medium flex items-center flex-wrap gap-x-2 gap-y-1">
             <span>Dùng tỷ lệ R:R tối thiểu 1:2 cho mỗi setup</span>
             <span className="text-slate-400">•</span>
             <span>Nếu thoát tay, ghi rõ lý do</span>
             <span className="text-slate-400">•</span>
             <span>Luyện tập đặt TP dựa trên plan từ trước</span>
           </div>
        </div>
