...
class OversizedBehavior extends BehaviorBase {
  ...
  buildEvidence(affectedTrades, classification, assetStats, topAffectedAsset) {
    ...
    if (classification === 'declared_violation') {
        ...
        ev.addContext(`Phân tích hiệu suất trên ${topAffectedAsset.asset}: Lệnh Normal đạt ${fmt(topAffectedAsset.normExp)}, nhưng lệnh Oversized (vượt P90) đạt ${fmt(topAffectedAsset.anomExp)} (Edge Delta: ${fmt(topAffectedAsset.delta)}).`);
    }
