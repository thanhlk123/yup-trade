...
class DcaBehavior extends BehaviorBase {
  ...
  buildEvidence(episodes, metrics, baseline, nonDcaBaseline, delta, classification) {
    ...
    ev.addContext(`Tổng quan về Averaging Down (Nhồi lỗ):`);
    ev.addContext(`${episodes.length} chu kỳ (Episodes) | ${metrics.wins}W - ${metrics.losses}L`);
    ...
    if (episodes.length >= 3) {
        ev.addContext(`Phân tích Edge (DCA vs Non-DCA):`);
        ev.addContext(`Win Rate: ${(metrics.winRate * 100).toFixed(1)}% vs ${(nonDcaBaseline.winRate * 100).toFixed(1)}% (Non-DCA)`);
        ev.addContext(`Profit Factor: ${metrics.profitFactor.toFixed(2)} vs ${nonDcaBaseline.profitFactor.toFixed(2)} (Non-DCA)`);
        ev.addContext(`Expectancy: ${metrics.expectancy.toFixed(2)} vs ${nonDcaBaseline.expectancy.toFixed(2)} (Non-DCA)`);

        ev.addContext(`Added Risk / Exposure (Gia tăng rủi ro):`);
        ev.addContext(`Trung bình bạn đã tăng khối lượng vị thế thêm ${((metrics.avgAddedSize / metrics.avgInitialSize) * 100).toFixed(0)}% sau khi lệnh ban đầu bị âm.`);

        if (classification === 'harmful') {
           ev.addContext(`ĐÁNH GIÁ (HARMFUL): Hành vi DCA đang phá hủy lợi thế giao dịch của bạn. Expectancy giảm mạnh so với những lệnh bạn không nhồi.`);
        } else if (classification === 'underperforming') {
           ev.addContext(`ĐÁNH GIÁ (UNDERPERFORMING): DCA vẫn có lãi, nhưng hiệu suất kém hơn đáng kể so với hệ thống giao dịch gốc của bạn.`);
        } else if (classification === 'effective') {
           ev.addContext(`ĐÁNH GIÁ (EFFECTIVE): Hệ thống ghi nhận DCA của bạn là một chiến lược Scale-in có Edge dương.`);
        } else {
           ev.addContext(`ĐÁNH GIÁ (NEUTRAL): DCA không gây ảnh hưởng tiêu cực đáng kể lên hệ thống của bạn.`);
        }
    } else {
        ev.addContext(`ĐÁNH GIÁ (INSUFFICIENT DATA): Bạn mới có ${episodes.length} chu kỳ DCA được ghi nhận. Chưa đủ dữ liệu để kết luận DCA là chiến lược có lợi hay gây hại.`);
    }
