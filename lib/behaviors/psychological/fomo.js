...
class FomoBehavior extends BehaviorBase {
  ...
  buildEvidence(affectedTrades) {
    ...
    // Derived
    if (inferredCount > 0) {
      ev.addContext(`Có ${inferredCount} lệnh không được khai báo nhưng hệ thống xếp loại "Strong Inference FOMO" do hội tụ từ 2 bằng chứng độc lập trở lên.`);
    }
    
    const postLossCount = affectedTrades.filter(a => a.context.postLoss).length;
    if (postLossCount > 0) {
      ev.addContext(`Trong đó, ${postLossCount} lệnh FOMO xảy ra ngay sau một lệnh thua lỗ (dấu hiệu của Post-loss Impulse).`);
    }
