...
export function parseTags(field) {
  ...
  try {
    ...
    // Not valid JSON array, fallback to string splitting
  }
  return String(field).split(',').map(t => t.trim()).filter(Boolean);
}

export function calculateInitialRisk(trade) {
  const sl = parseFloat(trade.stop_loss);
  const entry = parseFloat(trade.entry_price);
  const size = Math.abs(parseFloat(trade.size));
  if (isNaN(sl) || isNaN(entry) || isNaN(size)) return 0;
  
  // Note: we can't reliably know the contract multiplier here (e.g. XAUUSD = 100), 
  // so this returns "risk units" (price delta * size). 
  // For behavior comparison within the same asset, this relative metric is perfectly fine.
  return Math.abs(entry - sl) * size;
}

export function calculateMAD(arr) {
  if (arr.length === 0) return 0;
