...
class CompulsiveReEntryBehavior extends BehaviorBase {
  ...
  buildEvidence(episodes, maxGapMins) {
      ...
      ev.addContext(`${episodes.length} Re-entry Episodes`);
      ev.addContext(`Same asset + same direction`);
      ev.addContext(`Entry gap <${maxGapMins} minutes`);
      ev.addContext(`Triggered after LOSS`);
