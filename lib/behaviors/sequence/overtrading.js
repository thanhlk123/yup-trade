...
class OvertradingBehavior extends BehaviorBase {
  ...
  buildEvidence(episodes, metrics, baseline, threshold, classification, capacity, expDeltaRatio) {
      ...
      ev.addDerived(`Tổng quan về Giao dịch tần suất cao (Candidate Sessions):`);
      ev.addDerived(`${episodes.length} phiên (Sessions) | ${metrics.wins}W - ${metrics.losses}L`);
      ...
      if (classification !== 'insufficient_data') {
          ev.addDerived(`Phân tích Edge (Overtrading vs Bình thường Tinh khiết):`);
          ev.addDerived(`Expectancy (Primary): $${metrics.expectancy.toFixed(2)} vs $${baseline.expectancy.toFixed(2)} (Bình thường) -> Biến thiên ${(expDeltaRatio * 100).toFixed(1)}%`);
          ev.addDerived(`Profit Factor (Secondary): ${metrics.profitFactor.toFixed(2)} vs ${baseline.profitFactor.toFixed(2)}`);
          ev.addDerived(`Win Rate (Diagnostic): ${(metrics.winRate * 100).toFixed(1)}% vs ${(baseline.winRate * 100).toFixed(1)}%`);
          
          if (capacity && capacity.peakBucket) {
              if (capacity.tippingPoint) {
                  ev.addDerived(`DỮ LIỆU CAPACITY (COACHING): Lợi thế giao dịch của bạn đạt đỉnh ở mức ${capacity.peakBucket} lệnh/phiên (Expectancy $${capacity.peakExpectancy.toFixed(2)}). Nhưng khi bạn chạm hoặc vượt ngưỡng ~${capacity.tippingPoint} lệnh/phiên, lợi thế này bắt đầu bốc hơi và chuyển sang âm. Hãy lấy ${capacity.tippingPoint} làm giới hạn chịu đựng (Capacity) của não bộ bạn.`);
              } else {
                  ev.addDerived(`DỮ LIỆU CAPACITY (COACHING): Lợi thế giao dịch của bạn đạt đỉnh ở mức ${capacity.peakBucket} lệnh/phiên (Expectancy $${capacity.peakExpectancy.toFixed(2)}).`);
              }
          }

          if (classification === 'harmful') {
              ev.addDerived(`ĐÁNH GIÁ (HARMFUL): Hành vi say máu giao dịch đang phá hủy nghiêm trọng lợi thế của bạn (Expectancy giảm sâu <= -20%).`);
          } else if (classification === 'underperforming') {
              ev.addDerived(`ĐÁNH GIÁ (UNDERPERFORMING): Bạn vẫn có lãi trong các phiên tần suất cao, nhưng hiệu suất kém hơn đáng kể so với hệ thống giao dịch gốc.`);
          } else if (classification === 'effective') {
              ev.addDerived(`ĐÁNH GIÁ (EFFECTIVE): Tuyệt vời. Việc tăng tần suất đang mang lại Edge lớn hơn cho bạn. Bạn đang tận dụng tốt thanh khoản thị trường.`);
          } else {
              ev.addDerived(`ĐÁNH GIÁ (NEUTRAL): Tần suất giao dịch cao không làm thay đổi đáng kể lợi thế giao dịch của bạn.`);
          }
      } else {
          ev.addDerived(`ĐÁNH GIÁ (INSUFFICIENT DATA): Chưa đủ dữ liệu thống kê (Yêu cầu tối thiểu 10 tổng Sessions và 3 Overtrading Sessions) để kết luận tần suất cao gây hại hay có lợi.`);
      }
