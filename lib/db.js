...
export async function getDb() {
  ...
  await dbInstance.exec(`
    CREATE TABLE IF NOT EXISTS account_tabs (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  await dbInstance.exec(`
    CREATE TABLE IF NOT EXISTS tag_aliases (
      ...
      created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
      FOREIGN KEY (hashtag_id) REFERENCES custom_hashtags(id)
    )
  `);

  // Migration for existing databases
  try {
    await dbInstance.exec("ALTER TABLE custom_hashtags ADD COLUMN slug TEXT");
  } catch (e) {
  ...
  for (const col of newCols) {
    try {
      await dbInstance.exec(`ALTER TABLE trades ADD COLUMN ${col} TEXT`);
    } catch (e) {
      // Column already exists, ignore
    }
  }
  ...
  try {
    await dbInstance.exec("ALTER TABLE trades ADD COLUMN risk_amount REAL");
  } catch (e) {
    // Column already exists, ignore
  }

  // Coaching Memory table — stores AI coaching sessions for progress tracking
  await dbInstance.exec(`
    CREATE TABLE IF NOT EXISTS coaching_sessions (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      session_date TEXT NOT NULL,
      trade_type TEXT DEFAULT 'ALL',
      coach_focus TEXT,
      mission_items TEXT,
