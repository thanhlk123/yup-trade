...
export function getGeminiModels() {
  ...
  if (envModel) {
    ...
    cachedGeminiModels = Array.from(new Set(allModels));
  } else {
    cachedGeminiModels = defaultModels;
  }
  
  return cachedGeminiModels;
}

// Helper to remove empty values to save LLM tokens
export function cleanObject(obj) {
  ...
  if (typeof obj === 'object') {
    const cleaned = Object.entries(obj).reduce((acc, [key, val]) => {
      const c = cleanObject(val);
      if (c !== undefined) acc[key] = c;
      return acc;
    }, {});
...
export async function analyzeTradeWithAI(trade, lang = 'vi') {
  ...
  const ruleEngineOutput = system_inconsistencies.length > 0 ? `\n[RULE ENGINE - HARD FACTS]\n- DATA INCONSISTENCIES: ${system_inconsistencies.join(' | ')}\n` : '';
...
## RAW TRADE CONTEXT
${JSON.stringify(cleanObject(t) || {}, null, 2)}
...
`;
  ...
  if (responseText) {
    ...
    // Robustly extract JSON block if wrapped in markdown or contains leading/trailing text
    const jsonMatch = cleaned.match(/```(?:json)?\s*([\s\S]*?)\s*```/i);
    if (jsonMatch) {
      cleaned = jsonMatch[1];
    } else {
      // Fallback: extract the outermost JSON object if markdown is missing but there's conversational text
