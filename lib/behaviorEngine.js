...
function computePriorityScore(b) {
  ...
  // Handle confidence as either an object or a scalar
  const confVal = typeof b.confidence === 'object' && b.confidence !== null
    ? Math.max(b.confidence.statistical || 0, b.confidence.declared || 0)
    : (b.confidence || 0.5);
  const freqBonus = Math.min((b.occurrences || 0) * 10, 200);
  
  // To satisfy user's requirement: "Cái nào mất nhiều tiền thì lên đầu"
  // We make damage the dominant factor (x1000 multiplier), then add the regular score for tie-breaking
  return (damage * 1000) + (damage * severity * confVal) + freqBonus;
}
