...
export default function MonthlyReviewModal() {
  ...
  const isOpen = useSmartReviewStore(state => state.isMonthlyReviewOpen);
  const onClose = () => useSmartReviewStore.getState().setMonthlyReviewOpen(false);
  const monthlyReview = useSmartReviewStore(state => state.monthlyReview);

  if (!isOpen || !monthlyReview) return null;
        ...
        <div className="px-6 py-4 border-b border-slate-800 flex justify-between items-center bg-slate-950/50">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold text-white uppercase tracking-wider">
              {t('monthlyReviewTitle') || 'ĐÁNH GIÁ THÁNG NÀY'}
            </h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-slate-950/60 p-4 rounded-2xl border border-slate-850">
            <div className="text-center sm:border-r border-slate-850 flex flex-col justify-center py-2">
              <span className="text-[10px] text-slate-500 uppercase font-semibold">Tăng trưởng</span>
              <span className="text-3xl font-extrabold text-purple-400 font-mono mt-1">
                {monthlyReview.growth_rating || 'Trung bình'}
              </span>
            </div>
            <div className="sm:col-span-2 flex flex-col justify-center pl-2">
              <span className="text-[10px] text-slate-500 uppercase font-semibold mb-1">ĐÁNH GIÁ HỆ THỐNG</span>
              <p className="text-xs text-slate-350 leading-relaxed italic">
                &ldquo;{monthlyReview.system_evaluation}&rdquo;
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="bg-slate-950/40 border border-slate-850 p-4 rounded-xl space-y-2">
              ...
              <ul className="space-y-1.5 list-disc list-inside text-xs text-slate-300">
                {monthlyReview.risk_management_review?.map((r, i) => (
          ...
          <div className="bg-emerald-500/5 border border-emerald-500/10 p-4 rounded-xl space-y-2">
            <span className="text-emerald-450 text-[10px] font-bold uppercase tracking-wider block">
              🎯 Định hướng tháng sau
            </span>
            <ul className="space-y-1.5 list-disc list-inside text-xs text-slate-300">
              {monthlyReview.next_month_focus?.map((f, i) => (
