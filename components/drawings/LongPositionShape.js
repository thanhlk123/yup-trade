...
export default function LongPositionShape({ 
  drawing, 
  chart, 
  series, 
  isSelected, 
  ...
  // In Long Position, TP is above entry (lower Y), SL is below entry (higher Y)
  const tpHeight = Math.max(0, yEntry - yTP);
  const slHeight = Math.max(0, ySL - yEntry);

  const risk = pEntry - pSL;
  const reward = pTP - pEntry;
  ...
  return (
    <g>
      {/* TP Box */}
      <rect 
        x={left} y={yEntry - tpHeight} width={width} height={tpHeight} 
      ...
      {/* SL Box */}
      <rect 
        x={left} y={yEntry} width={width} height={slHeight} 
      ...
      {width > 60 && (
        <>
          <text x={left + width / 2} y={yEntry - tpHeight + 16} fill="#089981" fontSize="12" fontWeight="500" textAnchor="middle" style={{ pointerEvents: 'none', userSelect: 'none' }}>
            {pTP.toFixed(5)}
          </text>
          <text x={left + width / 2} y={yEntry - 6} fill={textColor} fontSize="11" textAnchor="middle" style={{ pointerEvents: 'none', userSelect: 'none' }}>
            Tỷ lệ R/R: {rr}
          </text>
          <text x={left + width / 2} y={yEntry + slHeight - 6} fill="#f23645" fontSize="12" fontWeight="500" textAnchor="middle" style={{ pointerEvents: 'none', userSelect: 'none' }}>
            {pSL.toFixed(5)}
          </text>
        </>
      )}
